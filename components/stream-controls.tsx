...
import { Play, Pause, Mountain, Waves, Shovel, Trees as Tree, Trash2, Grid3x3, RotateCcw } from "lucide-react"
...
export function StreamControls({ streamState, setStreamState }: StreamControlsProps) {
  const tools: { id: Tool; icon: typeof Shovel; label: string }[] = [
    ...
    { id: "plant", icon: Tree, label: "Add Vegetation" },
    { id: "remove", icon: Trash2, label: "Remove Plants" },
  ]

  return (
    <div className="absolute top-4 left-4 right-4 pointer-events-none flex flex-col gap-4 items-start">
      {/* Title */}
      ...
      <div className="flex gap-4 flex-wrap max-w-full">
        {/* Water Controls */}
        <Card className="pointer-events-auto bg-slate-800/95 border-slate-700 backdrop-blur-sm p-4 min-w-[280px]">
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              <Label className="text-slate-300 text-xs">Terrain Slope: {(streamState.slope * 100).toFixed(1)}%</Label>
              <Slider
      ...
      {/* Instructions */}
      <Card className="pointer-events-auto bg-slate-800/95 border-slate-700 backdrop-blur-sm p-3 max-w-md">
